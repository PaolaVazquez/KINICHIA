...
type GeminiResponse = {
  riskLevel?: string; riskScore?: number; confidence?: string; senderIntent?: string; threatTypes?: string[]; summary?: string; explanation?: string;
...
const SIGNAL_MAP: Record<string, FraudSignalType> = {
  ...
  payment_request: 'PAYMENT_REQUEST', financial_fraud: 'PAYMENT_REQUEST', suspicious_link: 'SUSPICIOUS_LINK', phishing: 'SUSPICIOUS_LINK',
  urgency: 'URGENCY', social_engineering: 'URGENCY', account_impersonation: 'ACCOUNT_IMPERSONATION', impersonation: 'ACCOUNT_IMPERSONATION',
  suspicious_refund: 'SUSPICIOUS_REFUND', payment_proof: 'PAYMENT_PROOF', delivery_fraud: 'IRREGULAR_DELIVERY', irregular_delivery: 'IRREGULAR_DELIVERY',
  marketplace_fraud: 'THIRD_PARTY_PAYMENT', third_party_payment: 'THIRD_PARTY_PAYMENT', concealment: 'CONCEALMENT',
};
...
export class KinichiaGeminiAnalyzer implements AnalysisEngine {
  ...
  async analyze(context: AnalysisContext): Promise<AnalysisResult> {
    ...
    for (const model of [...new Set([primary, fallback])]) {
      try {
        ...
        const response = await ai.models.generateContent({ model, contents: prompt, config: { systemInstruction: SYSTEM_INSTRUCTION, responseMimeType: 'application/json', ...(model.includes('3.8') ? { thinkingConfig: { thinkingLevel: ThinkingLevel.LOW } } : {}) } });
    ...
    if (!raw) throw new Error(`KINICHIA no pudo completar el análisis: ${lastError instanceof Error ? lastError.message : 'sin respuesta'}`);

    let parsed: GeminiResponse;
    try {
      parsed = JSON.parse(raw.replace(/^```json\s*/i, '').replace(/^```\s*/i, '').replace(/```$/i, '').trim()) as GeminiResponse;
    } catch {
  ...
  private normalizeSignal(signal: NonNullable<GeminiResponse['signals']>[number], recommendation?: string): FraudSignal | null {
    ...
    return { type, severity, score: severity === 'HIGH' ? 40 : severity === 'MEDIUM' ? 20 : 10, evidence: evidence.slice(0, 300), recommendation: recommendation || 'Verifica la solicitud mediante un canal oficial antes de compartir información o realizar pagos.' };
  }

  private localResult(riskLevel: AnalysisResult['riskLevel'], score: number, summary: string): AnalysisResult {
    return { riskLevel, score, summary, signals: [], recommendations: [], provider: 'GEMINI', modelName: 'none', engineVersion: '2.0.0' };
  }
